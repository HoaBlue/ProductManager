...
?>
    ...
    <div>
        ...
        $sqlLaptop = "SELECT Product.*, COUNT(Review.ReviewId) as num FROM Product LEFT JOIN Review ON Product.ProductId = Review.ProductId LEFT JOIN User ON Review.UserId = User.UserId JOIN ProductCategory ON Product.CategoryId = ProductCategory.CategoryId JOIN Supplier On Product.SupplierId = Supplier.SupplierId
                      WHERE ProductCategory.CategoryName LIKE 'Laptop' AND (Supplier.SupplierId = {$idSupplier} OR {$idSupplier} = -1)
                    ...
                    <ul class='pagination'>
                        ...
                        for($i = 0; $i < $pageCount;$i++){
                            ...
                            if($index == $currentPage){
                                ...
                                echo "<li><a href='laptop.php?idSupplier={$idSupplier}&pageIndex={$i}'>{$index}</a></li>";
                            }
                        }
                        if($count > ($pageIndex * $pageItem) + $pageItem){
                            echo "<li class='PagedList-skipToNext'><a href='laptop.php?idSupplier={$idSupplier}&pageIndex={$currentPage}' rel='next'>»</a></li> ";
                        }
                        ?>
